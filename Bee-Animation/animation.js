...
Animation.prototype.periodicRotation = function(model_transform, period, maxAngle) {
	var t = this.graphicsState.animation_time % period;					// time in milliseconds
	var rotAngle = 0;													// rotAngle in degrees

	// Calculate rotation angle as a function of time
	if ( t >= 0 && t < (1/4)*period ) { rotAngle = t / ( (1/4)*period / maxAngle ); }
	if ( t >= (1/4)*period && t < (1/2)*period ) { rotAngle = -(t - (1/4)*period) / ( (1/4)*period / maxAngle ) + maxAngle; }
	if ( t >= (1/2)*period && t < (3/4)*period) { rotAngle = -(t - (1/2)*period) / ( (1/4)*period / maxAngle ); }
	if ( t >= (3/4)*period && t <= period ) { rotAngle = (t - (3/4)*period) / ( (1/4)*period / maxAngle ) - maxAngle; }
	model_transform = mult( model_transform, rotation( rotAngle, 0, 0, 1 ) );
